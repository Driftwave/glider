...
class ServerData(object):
    _base_dir = '/Users/bsnyder/src/unsplash-crawler/photos'

    def __init__(self):
        self._collection_ids = ['unsplash']

        unsplash_image_ids = [fn[:-4] for fn in os.listdir(self._base_dir) if fn.endswith(".jpg")]
        self._image_ids = {'unsplash': unsplash_image_ids}
        self._generate_image_id2idx()

        self._weight_types = os.listdir(os.path.join(self._base_dir, 'weights'))
        self._image_weights = {wt: {} for wt in self._weight_types}
        for wt in self._weight_types:
            ws = []
            for img_id in self._image_ids['unsplash']:
                w = np.load(os.path.join(self._base_dir, 'feature_vectors', wt, img_id + '.npy'))
...
def calculate_probs(pos_ids, neg_ids, coll_id):
    ...
    if len(neg_ids) == 0:
        ...
        pos_weights = data.get_weights(DEFAULT_WEIGHT_TYPE, coll_id, image_ids=pos_ids)
        probs = nearest_neighbor_probs(pos_weights, coll_weights)

    elif len(pos_ids) == 0:
        assert len(neg_ids) > 0
        neg_weights = data.get_weights(DEFAULT_WEIGHT_TYPE, coll_id, image_ids=neg_ids)
...
DEFAULT_WEIGHT_TYPE = "inception_v3"
